...
const likePost =  async (req, res) => {
    ...
    await Like.create({
        ...
        if (error) throw error;
        Post.increment({
            likes: {by: 1, where: {post_id: post_id}},
        })
        res.status(200).json(results.row)
    })
}
...
// const viewHomePosts
const viewHomePosts = async (req, res) => {
    const user_id = req.params.id
    await Post.findAll({
        where: {
            user_id: user_id
...
const followUser =  async (req, res) => {
    ...
    await Like.create({
            follower_id: user_id,
            followee_id: followeeUserId
        }, (error, results) => {
        if (error) throw error;
        User.increment({
            following: {by: 1, where: {user_id: user_id}},
            followers: {by: 1, where: {user_id: followeeUserId}}
        })
        res.status(200).json(results.row)
    })
}
